demultiplexDoublePairedforENA <- function(primersFile, t2s, fastqR1In, fastqR2In, allowedMis = 0, outputFolder = "demult", fwdTagLength = 8, revTagLength = 8,
                                    overwrite = F, withIndels = F, openEnded = NULL, splitHeader = " ", chunkSize = 1000000, progressBar = T) {

  ## demultiplexDoublePairedforENA: We keep mixed orientation and keep the primer in the read (but we trim the tag), 
  ## allowing anyone to re-orient the reads on raw data submited to ENA 
  ...
  repeat {
    ...
    for (fwdComb in fwdCombs)
    {
      ...
      ## --> here we don't want to re-orient, so we fetch reads that have fwd and the ones that had rev in R1. 
      
      # indexing fwd in the R1
      ...
      tmpR1 <- fqR1IN_[which(xxyy==1)]
      tmpR2 <- fqR2IN_[which(xxyy==1)]
      ...
      for (revComb in t2s_fwdComb$reverse)
      {
        ...
        # indexing rev in the R1
        idx <- vmatchPattern2(rev, sread(narrow(tmpR1, start = 1, end = end.search.rev)), 
                              fixed=FALSE, max.mismatch=allowedMis, with.indels = withIndels)
        # get the indexing
        xx <- elementNROWS(idx)
        # indexing rev in the R2
        idy <- vmatchPattern2(rev, sread(narrow(tmpR2, start = 1, end = end.search.rev)), 
                              fixed=FALSE, max.mismatch=allowedMis, with.indels = withIndels)
        # get the indexing
        yy <- elementNROWS(idy)
        ...
        tmpR1_ <- tmpR1[which(xxyy==1)]
        tmpR2_ <- tmpR2[which(xxyy==1)]
        ...
        if (is.null(openEnded)) {
        	writeFastq(narrow(tmpR1_N, start = fwdTagLength+1, end = width(tmpR1_N)), paste0(outputFolder,"/", sample, "_R1.fastq.gz"), "a")
        	writeFastq(narrow(tmpR2_N, start = revTagLength+1, end = width(tmpR2_N)), paste0(outputFolder,"/", sample, "_R2.fastq.gz"), "a")
        ## or export where primers was found in "openEnded" window
        } else if (!is.null(openEnded)) {
          ## we need to re-index :/ 
          idx <- vmatchPattern2(fwd, sread(narrow(tmpR1_N, start = 1, end = end.search.fwd)), fixed=FALSE, max.mismatch=allowedMis, with.indels = withIndels)
          idy <- vmatchPattern2(rev, sread(narrow(tmpR2_N, start = 1, end = end.search.rev)), fixed=FALSE, max.mismatch=allowedMis, with.indels = withIndels)
          ## now export 
        	writeFastq(narrow(tmpR1_N, start = fwdTagLength+1, end = width(tmpR1_N)), paste0(outputFolder,"/", sample, "_R1.fastq.gz"), "a")
        	writeFastq(narrow(tmpR2_N, start = revTagLength+1, end = width(tmpR2_N)), paste0(outputFolder,"/", sample, "_R2.fastq.gz"), "a")
        }
